...
By default a read returns every channel of the dataset, one after another. `channel=<n>` returns channel `n` only (counting from 0), and can be combined with the other commands, e.g. `image+channel=1&project=8`. A channel the dataset does not have, a parameter that is not a whole number, or a `channel` item that cannot be parsed (no value, as in `image+channel=`, or a second `+` in the name) answers `400`. The `info` file is unchanged.
...
| `SEG_GZIP` | `0` (off) | The zlib level, `1` (fastest) to `9` (smallest), at which a successful image read of a writable layer (one with a `.sisf_access` file, which holds segmentation labels) is sent with `Content-Encoding: gzip` and `Vary: Accept-Encoding` to a client whose `Accept-Encoding` accepts `gzip`. Image layers, reads that carry a token (the portal backend's own, from the same host), every other route, errors, clients that do not accept `gzip`, bodies under 1024 bytes and bodies that would not get shorter are sent uncompressed, and so is a read whose compression fails. A value that is not a level from 0 to 9 is ignored with a log line. The read's time in `/performance` runs until `res.end()` returns, so it includes the compression, and it includes sending the body only when the body sent is 1 MiB or more (crow sends those inside `res.end()`, smaller ones after it): compressing a large read below 1 MiB also takes its send time out of that figure. |
